...
/**
 * View model for a map entry of a root type to all reference chains of that
 * type.
 *
 * @see ReferenceChain
...
public class ReferenceChainSet extends ViewModel {
    ...
    public ReferenceChainSet(QueryHandler handler, Map.Entry<Integer, Collection<ReferenceChain>> entry) {
        super(handler);
        this.entry = entry;
    }
    ...
    public Iterable<ReferenceChainView> getReferenceChains() {
        return new StreamIterable<>(entry.getValue().stream()
                ...
                .map(ref -> new ReferenceChainView(handler, ref)));
    }
}
